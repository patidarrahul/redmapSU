...
# visco capillary model for slot die coating
COLORS = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan']
# Variables to track points
point_counter = 1
points = []
def calculate_ca(request):
    global point_counter, points

    if request.method == 'POST':
        mu = float(request.POST['mu'])
        v = float(request.POST['v']) # in m/min
        # v in m/seconds
        v =  v / 60

        sigma = float(request.POST['sigma'])
        # q = float(request.POST['q'])
        wft = float(request.POST['wft'])
        gap = float(request.POST['gap'])

        label = request.POST['name']

        # Calculate results
        capillary_number = (mu * v) / sigma
        wet_thickness = wft * 10000  # Wet thickness in cm

        # Add the new point
        ho_t = gap / wet_thickness
        color = COLORS[(point_counter - 1) % len(COLORS)]
        points.append({'x': ho_t, 'y': capillary_number, 'color': color, 'label': f'{label}'})
        point_counter += 1

        # Generate plot
        fig = generate_plot(points)

        return render(request, 'visco-capillary-model.html', {
            'capillary_number': capillary_number,
            'wet_thickness': wet_thickness,
            'plot_div': fig.to_html()
        })

    # Initial empty plot
    fig = generate_plot(points)
    return render(request, 'visco-capillary-model.html', {'plot_div': fig.to_html()})
...
def reset_graph(request):
    ...
    point_counter = 1
    points = []
    return calculate_ca(request)


def generate_plot(points):
    # Initial Visco-Capillary model
    G = np.linspace(10, 100, 50)
    Ca_VCap = 0.65 * (2 / (G - 1)) ** 1.5
    ...
    fig.add_trace(go.Scatter(
        ...
        y=Ca_VCap,
        mode='lines',
        name="Stable Region",  # Label in legend for the shaded area
        fill='tozeroy',  # Fill to x-axis
        fillcolor='rgba(0, 100, 250, 0.2)'  # Semi-transparent blue
    ))
    ...
    for point in points:
        fig.add_trace(go.Scatter(
            ...
            mode='markers+text',
            text=point['label'],
            marker=dict(color=point['color'], size=10),
            name=point['label']
        ))

    # Set layout
    fig.update_layout(
        title="Visco-Capillary Model",
        xaxis_title="Dimensionless Gap [Ho/t]",
        yaxis_title="Capillary Number [Ca]",
        template="plotly_white"
    )
    return fig